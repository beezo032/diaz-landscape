...
// Data structure for the local case studies
const localProjects = [
  {
    id: 1,
...
export default function GalleryPage() {
  return (
    <div className="relative min-h-[100dvh] bg-brand-offwhite text-brand-forest pb-32">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-24">
        ...
        <div className="mb-24 md:mb-32">
          ...
          />
          <p className="text-center text-sm font-medium text-brand-forest-light mt-6 uppercase tracking-widest">
          ...
          <div className="columns-1 md:columns-2 lg:columns-3 gap-6 md:gap-8 space-y-6 md:space-y-8">
            {localProjects.map((project) => (
              ...
              >
                ...
                {/* Case Study Meta */}
                <div className="px-2">
                  <span className="inline-block px-3 py-1 mb-2 text-xs font-bold uppercase tracking-wider text-brand-forest bg-brand-grass rounded-full">
                    {project.tag}
                  </span>
                  <h3 className="text-xl font-semibold leading-tight text-brand-forest group-hover:text-brand-grass-dark transition-colors">
                    {project.title}
                  </h3>
                </div>
              </div>
      ...
      <div className="fixed bottom-0 left-0 w-full z-[9998] bg-brand-forest text-white border-t border-brand-grass/20 shadow-[0_-10px_40px_rgba(0,0,0,0.1)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-brand-offwhite/80">Available 24 hours in Ladson & Charleston.</p>
          </div>
          <Link 
            href="/#quote"
            className="w-full sm:w-auto inline-flex items-center justify-center gap-2 bg-brand-grass hover:bg-brand-grass-dark text-brand-forest font-bold px-8 py-3.5 rounded-lg transition-all hover:scale-105 active:scale-95 shadow-lg shadow-brand-grass/20"
          >
